 ...
 * NFT or xNFT. Mirrors behavior of useNFT hook but for server-side rendering.
...
export const fetchNFTData = async ({
  ...
  if (
    contractAddress &&
    contractAddress !== TULI_MEDIA_CONTRACT_BY_NETWORK[fetchAgent.networkId]
  ) {
    ...
    const nft = await fetchAgent.loadXNFTData(tokenId);
...
/**
 * Server-side initial data hook for xNFTGroup data hook
 * 
 * @param ids list of ids (addresses for creator or owner, xnft id for NFT)
 * @param type type of 'id' or 'creator' or 'owner' to determine what type of data to fetch
 * @returns NFTDataType
 */
export const fetchXNFTGroupData = async ({
  ids,
  type,
  fetchAgent,
  prepareDataJSON = true,
}: fetchXNFTGroupDataType) => {
  const nftGroup = await fetchAgent.fetchXNFTGroupData(ids, type);
