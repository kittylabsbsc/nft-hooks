...
import { CurrencyLookupType, NFTDataType, XNFTMediaDataType } from './AuctionInfoTypes';
import {
  OpenseaResponse,
...
export class MediaFetchAgent {
  ...
  private loaders: {
    // fetches NFT data from Tuli subgraph, cached and batched
    mediaLoader: DataLoader<string, XNFTMediaDataType>;
    ...
    // auctionIfnoLoader fetches auction info for non-tuli NFTs
  ...
  constructor(network: NetworkIDs) {
    ...
    this.loaders = {
      ...
      usernameLoader: new DataLoader((keys) => this.fetchTuliUsernames(keys)),
  ...
  /**
   * Un-batched fetch function to fetch a group of XNFT data
   *
   * @param ids list of ids to query
   * @param type type of ids: creator, id (of media), owner
   * @returns
   */
  async fetchXNFTGroupData(ids: string[], type: FetchGroupTypes) {
  ...
  /**
   * Get on-chain TULI NFT ID associated media information
   *
   * @param mediaId TULI NFT id to retrieve information of
  ...
  async loadXNFTDataUntransformed(mediaId: string) {
   ...
   * @private
   * @param curatorIds list of Tuli NFT IDs to fetch from the graph datastore
   ...
   * @private
   * @param mediaIds list of Tuli NFT IDs to fetch from the graph datastore
   * @returns mapped transformed list of tuli NFT ID data
   */
  ...
  /**
   * Fetches tuli username information from blockchain addresses for displaying user
   * information.
   *
   * @param addresses string list of addresses to map to Tuli usernames
   * @returns list of UsernameResponseType - all fields are optional except address
   */
  private async fetchTuliUsernames(addresses: readonly string[]) {
    const fetchWithTimeout = new FetchWithTimeout(this.timeouts.Tuli);
    const response = await fetchWithTimeout.fetch(TULI_USERNAME_API_URL, {
      method: 'POST',
      type: 'cors',
      headers: {
  ...
  public async fetchIPFSMetadata(url: string) {
    // TODO(iain): Properly parse metadata from `ourtuli/media-metadata-schemas
